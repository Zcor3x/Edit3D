...
namespace Gibbed.Prototype.FileFormats
{
    public class Pure3DFile
    {
        ...
        private static byte[] CopyStream(Stream input)
        {
        ...
        private static Pure3D.BaseNode DeserializeNode(Stream input, Endian endian, Pure3D.BaseNode parent)
        {
            ...
            if (current != null)
            {
                ...
                long _startPosition = input.Position;
                long _endPosition = 0;

                current.Deserialize(input);

                _endPosition = input.Position;

                long _bytesToRead = _endPosition - _startPosition;

                current.MemoryDump = new byte[_bytesToRead];

                input.Position = _startPosition;
                input.Read(current.MemoryDump, 0, (int)_bytesToRead);

                input.Position = _endPosition;
            ...
            else
            {
                ...
                current = unknown;
                current.MemoryDump = unknown.Data;
            }
