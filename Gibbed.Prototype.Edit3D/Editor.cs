...
namespace Gibbed.Prototype.Edit3D
{
    public partial class Editor : Form
    {
        ...
        private void HexViewer_Click(object sender, EventArgs e)
        {
            ...
            if (node.MemoryDump != null && node.MemoryDump.Length > 0)
            {
                G1zm0Pure3DLink.CreateLink(node.MemoryDump);
            }
            else
            {
                MessageBox.Show("G1zm0 says: Can't read stream.", "Unable to Read", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
